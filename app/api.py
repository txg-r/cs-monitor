...
def list_events(
    ...
    return list(db.scalars(_event_query(
        ...
        "evidence": [
            {
                ...
                "published_at": link.raw_item.published_at,
            }
...
def daily_reports(limit: int = Query(default=30, ge=1, le=100), db: Session = Depends(get_db)) -> list[DailyReport]:
    return list(db.scalars(select(DailyReport).order_by(DailyReport.report_date.desc()).limit(limit)).all())
